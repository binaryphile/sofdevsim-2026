...
func TestProjection_Apply_TicketAssigned(t *testing.T) {
	...
	// Assign at tick 5
	startedAt := time.Now()
	evt := events.NewTicketAssigned("sim-1", 5, "TKT-001", "dev-1", model.PhaseResearch, startedAt)
	got := proj.Apply(evt)
...
func TestProjection_Apply_BufferConsumed_FeverTransitions(t *testing.T) {
	...
	proj = proj.Apply(events.NewTicketAssigned("sim-1", 1, "T-2", "dev-1", model.PhaseResearch, time.Time{}))
